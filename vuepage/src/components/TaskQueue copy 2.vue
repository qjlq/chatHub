  ...
  methods: {
    ...
    async initTableResize() {
      ...
      this.$nextTick(() => {
        if (this.$refs.table) {
          // 添加防抖处理
          this.debouncedDoLayout = this.debounce(() => {
            this.$refs.table.doLayout()
          }, 100)
          // 使用 ResizeObserver 替代原有方式
          
          this.observer = new ResizeObserver(this.debouncedDoLayout)
